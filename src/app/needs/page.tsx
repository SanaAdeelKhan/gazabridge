                ...
                >
                    ...
                    {user ? (
                      <>
                        ...
                        <div style={{ fontSize: '0.78rem', color: '#9ca3af', marginTop: '2px' }}>
                          ...
                          {req.profiles?.gender ? ` · ${req.profiles.gender}` : ''}
                        </div>
                      </>
                    ) : (
                      <div style={{ fontWeight: 700, fontSize: '0.95rem', color: '#1a1a2e' }}>{req.profiles?.name}</div>
                    )}

                    {/* Languages */}
                    {user && (req.profiles?.languages?.length ?? 0) > 0 && (
                  ...
                  <div style={{ flex: '0 0 auto', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    {user ? (
                      <>
                        <button
                          onClick={() => handleOfferHelp(req.profiles?.id)}
                        ...
                        {isVolunteer && (
                          <button
                            onClick={() => handleGiftCourse(req.profiles?.id)}
                        ...
                        {req.profiles?.whatsapp && (
                          <a
                            href={req.profiles.whatsapp.startsWith('http') ? req.profiles.whatsapp : `https://wa.me/${req.profiles.whatsapp.replace(/[^0-9]/g, '')}`}
