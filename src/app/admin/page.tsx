...
type Profile = { id: string; name: string; email?: string; is_volunteer: boolean; is_seeker: boolean; is_admin: boolean; is_vetted: boolean; country: string; status: string }
...
export default function AdminPage() {
  ...
  async function fetchProfiles() {
    const { data } = await supabase
      .from('profiles')
      .select("id, name, is_volunteer, is_seeker, is_admin, is_vetted, country, status")
  ...
  async function toggleVetted(profileId: string, current: boolean) {
    const { error } = await supabase.from("profiles").update({ is_vetted: !current }).eq("id", profileId)
    if (error) console.error('Failed to update vetted status', error)
    fetchProfiles()
  }

  async function toggleAdmin(profileId: string, current: boolean) {
    const { error } = await supabase.from('profiles').update({ is_admin: !current }).eq('id', profileId)
    if (error) console.error('Failed to update admin status', error)
    fetchProfiles()
  }

  async function updateStatus(profileId: string, status: 'active' | 'suspended' | 'banned') {
    const { error } = await supabase.from('profiles').update({ status }).eq('id', profileId)
    if (error) { console.error('Failed to update status', error); return }
    setSuccess(`✅ User ${status === 'active' ? 'restored' : status} successfully!`)
  ...
  const suspended = profiles.filter(p => p.status === 'suspended' || p.status === 'banned')
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '48px 24px', fontFamily: 'inherit' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '32px' }}>
          {[
            ...
            { label: 'Suspended', value: suspended.length, color: '#dc2626', bg: '#fef2f2' },
          ].map(s => (
          ...
          <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #fde68a', padding: '28px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {profiles.map(p => {
                ...
                return (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderRadius: '12px', background: isBanned ? '#fef2f2' : isSuspended ? '#fff7ed' : '#f9fafb', border: isBanned ? '1px solid #fecaca' : isSuspended ? '1px solid #fed7aa' : '1px solid transparent', flexWrap: 'wrap' }}>
                    <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: isBanned ? 'linear-gradient(135deg,#dc2626,#ef4444)' : isSuspended ? 'linear-gradient(135deg,#d97706,#f59e0b)' : 'linear-gradient(135deg,#d97706,#f59e0b)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '0.9rem', flexShrink: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                      ...
                      {isSuspended && <span style={{ fontSize: '0.7rem', background: '#fff7ed', color: '#c2410c', padding: '2px 10px', borderRadius: '100px', fontWeight: 600 }}>⏸ Suspended</span>}
                      {isBanned && <span style={{ fontSize: '0.7rem', background: '#fef2f2', color: '#dc2626', padding: '2px 10px', borderRadius: '100px', fontWeight: 600 }}>🚫 Banned</span>}
                    </div>
                    {!isMe && (
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {(isSuspended || isBanned) ? (
                          <button onClick={() => updateStatus(p.id, 'active')} style={{ padding: '6px 14px', borderRadius: '100px', fontFamily: 'inherit', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0' }}>
                            ✅ Restore
                          </button>
                        ) : (
                          <>
                            <button onClick={() => updateStatus(p.id, 'suspended')} style={{ padding: '6px 14px', borderRadius: '100px', fontFamily: 'inherit', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, background: '#fff7ed', color: '#c2410c', border: '1px solid #fed7aa' }}>
                              ⏸ Suspend
                            </button>
                            <button onClick={() => updateStatus(p.id, 'banned')} style={{ padding: '6px 14px', borderRadius: '100px', fontFamily: 'inherit', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca' }}>
                              🚫 Ban
                            </button>
                          </>
                        )}
                    ...
                    {isMe && <span style={{ fontSize: '0.75rem', color: '#9ca3af' }}>You</span>}
                  </div>
                )
              })}
            </div>
